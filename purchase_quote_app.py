...
def load_gsheet(spreadsheet_id: str, sheet_name: str) -> pd.DataFrame:
    """Load data from a specific Google Sheet worksheet."""
    ...
    try:
        ...
        st.warning(f"⚠️ Could not load '{sheet_name}' from Google Sheets: {e}")
        return pd.DataFrame()
...
def load_quotes() -> pd.DataFrame:
    if not gc: return pd.DataFrame(columns=REQUIRED_COLS)
    try:
        ...
        st.error(f"❌ Error loading quotes from Google Sheets: {e}")
        return pd.DataFrame(columns=REQUIRED_COLS)
